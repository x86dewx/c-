...
/***************************************************************************************************************/
                    ...
                    if(n == 3)
                    {
                        ...
                        socklen_t addrlen =sizeof(seraddr);
                        connect(fd,(const struct sockaddr*)&seraddr,addrlen);
                        char buf3[8][1024] ={"GET /?app=weather.future&cityNm=西安&appkey=73838&sign=dfd37a7528fa52b7190b44492789ceee&format=json HTTP/1.1\r\n",
                            "Host: api.k780.com\r\n",
                            "User-Agent: Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:109.0) Gecko/20100101 Firefox/113.0\r\n",
                            "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8\r\n",
                            "Accept-Language: zh-CN,zh;q=0.8,zh-TW;q=0.7,zh-HK;q=0.5,en-US;q=0.3,en;q=0.2\r\n",
                            ...
                            "Connection: close\r\n",
                            "Upgrade-Insecure-Requests: 1\r\n\r\n"};
                        sprintf(buf3[0],"GET /?app=weather.future&cityNm=%s&appkey=73838&sign=dfd37a7528fa52b7190b44492789ceee&format=json HTTP/1.1\r\n",buf);
                        for(int i = 0;i <8;++i)
                        {
                            write(fd,buf3[i],strlen(buf3[i]));
                        }
                        ...
                        char *buf4=(char*)malloc(10240) ;
                        bzero(buf4,sizeof(buf4));
                        char buf5[1024] ={0};
                        while(1)
                        {
                            bzero(buf5,sizeof(buf5));
                            int ret =read(fd,buf5,sizeof(buf5)-1);
                            if(ret <= 0)
                            {
                                break;
                            }
                        ...
                        buf4[num2 - 2]='\0';
                        printf("%s",buf4);
                        cJSON* cjson_future = NULL;
                        cJSON* cjson_success = NULL;
                        int result_array_size = 0, i = 0;
                        cJSON* cjson_result_item =NULL;
                        cJSON* cjson_result_item_days =NULL;
                        ...
                        cjson_future = cJSON_Parse(buf4);
                        if(cjson_future == NULL)
                        {
                            printf("parse fail.\n");
                            return -1;
                        }

                        cjson_success = cJSON_GetObjectItem(cjson_future, "success");
                        if(cjson_success == NULL)
                        {
                            flag1 = 1;
                            bzero(buf,sizeof(buf));
                        ...
                        printf("%s",cjson_success->valuestring);

                        
                        window3(2);

                        cjson_result =cJSON_GetObjectItem(cjson_future,"result");
                        result_array_size =cJSON_GetArraySize(cjson_result);
...
//part2 实时天气查询数据打印部分
                        ...
                        {
                            cjson_result_item =cJSON_GetArrayItem(cjson_result,i);
                            cjson_result_item_days = cJSON_GetObjectItem(cjson_result_item,"days");
                            printf("\033[0m");
                            printf("\033[%d;74H",15+i);
                            printf("\033[93;104m");
                            printf("%s", cjson_result_item_days->valuestring);
                            printf("\033[0m");
                        }
